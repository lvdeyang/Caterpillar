...
<script type="text/javascript">
	...
	$(function() {
      ...
      var _uriRegion = window.BASEPATH + 'distributor/query/region';
		...
		$.get(_uriRegion, null, function(data){
			...
			if(data && data.length>0){
				...
				for(var i=0; i<data.length; i++){
				    var cssclass='';
				    if(data[i].code==regionCode){
				    	cssclass='current';
				    	$('#currentRegion').html(data[i].name);
				    	$('#currentSecRegion').html(data[i].name);
				    }
					html.push('<a class="region '+cssclass+'" id="region-'+data[i].code+'" href="javascript:void(0)">'+data[i].name+'</a> ');
				}
		...
		//获取所有一级推荐
      var _uriRecomment = window.BASEPATH + 'distributor/query/recomments';
		...
		$.get(_uriRecomment, null, function(data){
			...
			if(data && data.length>0){
			    var html=[];
				for(var i=0; i<data.length; i++){
					html.push('<div class="swiper-slide"><img src="" alt=""></div>');
				}
		...
		//获取所有二级推荐
      var _uriSecRecomment = window.BASEPATH + 'distributor/query/secrecomments';
		...
		$.get(_uriSecRecomment, null, function(data){
			...
			if(data && data.length>0){
			    var html=[];
				 html.push('<tr><td style="border-top-left-radius:10px;" rowspan="2"><span id="currentSecRegion">河北<</span>全部景点</td><td>万佛园</td><td style="border-top-right-radius:10px;">清东陵</td></tr>');
		         html.push('<tr><td>古北水镇</td><td>欢乐谷</td></tr>');
		         html.push('<tr><td style="border-bottom-left-radius:10px;">汤泉宫</td><td>禅林寺</td><td style="border-bottom-right-radius:10px;">长城</td></tr>');
			}
			...
			//获取所有三级推荐
      var _uriThirdRecomment = window.BASEPATH + 'distributor/query/thirdcomments';
		...
		$.get(_uriThirdRecomment, null, function(data){
			...
			if(data && data.length>0){
				...
				for(var i=0; i<data.length; i++){
					...
					html.push('	<img style="width:100%;height:100%" class="weui-media-box__thumb" src="/image/1.jpg" alt="">');
					html.push('</div>');
					html.push('<div class="weui-media-box__bd">');
					html.push('	<h4 style="width:50%;float:left" class="weui-media-box__title">清东陵</h4><h4 style="width:50%;float:right;text-align:right" class="weui-media-box__title">$140</h4>');
					html.push('	<p style="width:100%;height:15px" class="weui-media-box__desc">由各种物质组成的巨型球状天体，叫做星球。星球有一定的形状，有自己的运行轨道。</p>');
					html.push('	<p style="width:100%" class="weui-media-box__desc"><div style="float:right;margin-right:4px;">抢购</div></p>');
					html.push('</div>');
				    html.push('</a>');
				}
	...
	<div id="page">
		...
		<div class="content">
		    ...
		    <table id="columnTable">
		         <!-- <tr><td style="border-top-left-radius:10px;" rowspan="2"><span id="currentSecRegion">河北<</span>全部景点</td><td>万佛园</td><td style="border-top-right-radius:10px;">清东陵</td></tr>
		         <tr><td>古北水镇</td><td>欢乐谷</td></tr>
		         <tr><td style="border-bottom-left-radius:10px;">汤泉宫</td><td>禅林寺</td><td style="border-bottom-right-radius:10px;">长城</td></tr>-->
		    </table>
		    
		    <div style="width:100%;text-align:center;margin-top:5px;">——<span id="currentRegion">河北</span>精选——</div>
		    ...
		    <div style="margin-top:5px" class="weui-panel__bd" id="thirdRecomments">
				<!-- <a href="javascript:void(0);"
					...
					<div class="weui-media-box__bd">
						<h4 style="width:50%;float:left" class="weui-media-box__title">清东陵</h4><h4 style="width:50%;float:right;text-align:right" class="weui-media-box__title">$140</h4>
						<p style="width:100%;height:15px" class="weui-media-box__desc">由各种物质组成的巨型球状天体，叫做星球。星球有一定的形状，有自己的运行轨道。</p>
						<p style="width:100%" class="weui-media-box__desc"><div style="float:right;margin-right:4px;">抢购</div></p>
					</div>
				</a>  -->
