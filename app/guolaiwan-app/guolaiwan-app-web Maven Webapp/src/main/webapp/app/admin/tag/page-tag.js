...
define([
    ...
    var doable = function(){
        if(_unsaved === true){
            commons.messanger.info('当前有未保存的菜单，请先保存!');
            return false;
        }
        return true;
    }
    ...
    var _init = function(){
        ...
        var _uri = 'tag/get/' + context.model.user.id;
        ...
        commons.ajax.get(_uri, null, function(data){
            ...
            var toggle_button = function(){
                ...
                $table.find('input[name=tag]').each(function(){
                    var $checkbox = $(this);
            ...
            //绑定选中事件
            $table.on('ifChecked', 'input[name=tag]', function(){
                var $checkbox = $(this);
            ...
            $table.on('ifUnchecked', 'input[name=tag]', function(){
                var $checkbox = $(this);
                if(!$checkbox.is('checkall')){
            ...
            $button_add.on('click', function(){
                ...
                _table_menu.row.add([
                    '-',
                    '<div class="col-lg-4 col-md-6 col-sm-8 col-xs-12" style="padding:0; margin:0; position:relative;"><input type="text" class="form-control tag-name" placeholder="标签名称" required style="width:100%; border-radius:4px;"/></div>',
                    date,
                ...
                $tr.on('click.save', '.save', function(){
                    ...
                    var _uri = 'tag/add/' + context.model.user.id;
                    ...
                    commons.ajax.post(_uri, {
                        ...
                        //新增数据行
                        _table_menu.row.add([
                            '<input type="checkbox" name="tag" class="flat-red" value="'+data.tag.id+'"/>',
                            data.tag.name,
                            data.tag.updateTime,
                            '<a class="edit" data-toggle="tooltip" data-placement="bottom" title="编辑菜单"><i class="fa fa-edit"></i></a>'+
                            '<a class="last remove" data-toggle="tooltip" data-placement="bottom" title="删除菜单"><i class="fa fa-trash-o"></i></a>'
                        ]).draw();
            ...
            $button_remove.on('click', function(){
                ...
                $table.find('input[name=tag]').each(function(){
                    var $checkbox = $(this);
                    if(!$checkbox.is('.checkall') && $checkbox.is(':checked')){
                        _tagIds.push($checkbox.val());
                    }
                });

                var _uri = 'tag/remove/' + context.model.user.id
                ...
                commons.ajax.delete(_uri, {
                    ...
                    commons.messanger.success('操作成功，当前删除：'+_tagIds.length+'条数据。');
