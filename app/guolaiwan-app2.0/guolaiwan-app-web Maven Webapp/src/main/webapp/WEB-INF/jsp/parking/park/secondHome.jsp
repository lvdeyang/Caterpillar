...
<script type="text/javascript">
	...
	$(function() {
		...
		$.post(_url, null, function(data) {
			data = parseAjaxResult(data);
			var htm = [];
			 htm.push('<span id="alert_a">' + data.userHeadimg + ' </span>' + '的')
			htm.push('<span id="alert_b">' + data.userNickname + '</span>' + '，在') 
			$('#alerts').append(htm.join(''));
		})	;
		...
		$.post(_ur, null, function(data) {
			...
			var html = [];
			html.push('<span id="alert_c">' + data[1].parkingName + '</span>' + '，')
			html.push('<span id="alert_d">' + data[1].parkingDistrict + '</span>')
			html.push('<span id="alert_e">' + data[1].parkingNumber + '</span>')
		    $('#alerts').append(html.join(''));
		  /*  } */
        ...
        var uri = window.BASEPATH + 'vice/finish';
		$.post(uri, null, function(data) { 
		  data = parseAjaxResult(data);
		  for(var i=0; i<data.length ; i++){
			//30分钟弹提示框
			var shi = new Date(data[i]);
			var today = new Date();
			var tan = ((shi - today) / 1000 / 60).toFixed(2);
			if (tan < 0) {
				myAlert('系统提示:', '您已超时，当前系统正在按停车场三倍停车费用为您计时，请您及时续费。');
			} else if (tan < 30) {
				myAlert('系统提示:', '<span id="alerts">您的车牌号<span id="alert_a"></span><span id="alert_b"></span><span id="alert_c"></span><span id="alert_d"></span><span id="alert_e"></span></span>的停车时间将于30分钟后结束。如果您需要续费，请在预订时间结束前30分钟内续费。如果您预订的车位超出预订时间，未能驶离停车场，系统将按照三倍每小时费用自动计算。');
			} 
		  }
        });
