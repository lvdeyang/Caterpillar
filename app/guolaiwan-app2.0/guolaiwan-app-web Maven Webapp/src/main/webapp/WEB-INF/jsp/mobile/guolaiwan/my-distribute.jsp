...
<script type="text/javascript">
	...
	$(function() {
		...
		var _uriProduct = window.BASEPATH + 'distributor/query/dispro/${distributorId}';
        ...
        var _urionlineProduct = window.BASEPATH + 'distributor/query/onlinedispro/${distributorId}';
		
		$.get(_urionlineProduct, null, function(data){
			data = parseAjaxResult(data);
			if(data === -1) return;
			if(data && data.length>0){
			    var html=[];
				...
				$('#product_table_online').append(html.join(''));
		...
		function changeStatus(id,status){
				 var _urichangeStatus = window.BASEPATH + 'distributor/change/onlineStatus/'+id+'/'+status;
				
					$.get(_urichangeStatus, null, function(data){
						data = parseAjaxResult(data);
						if(data === -1) return;
					    if(status==0){
					       $.toast("下架成功");
	...
	<div id="page">
		...
		<div class="header">
			<div class="wrapper">
				...
				<div class="header-content">分销商品</div>
			</div>
		</div>
		<div class="content">
			...
			<div class="weui-tab">
				<div class="weui-navbar" style="height:40px;line-height:14px;">
					<a onclick="return false" class="weui-navbar__item weui-bar__item--on" href="#tab1">
						全部商品 </a> <a onclick="return false" class="weui-navbar__item" href="#tab2"> 已上架 </a>
				</div>
