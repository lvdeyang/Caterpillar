	...
	$(function() {
      ...
      var _uriAddress = window.BASEPATH + 'phoneApp/address/list?userId=${userId}';
		...
		$.get(_uriAddress, null, function(data){
			...
			if(data && data.length>0){
				...
				for(var i=0; i<data.length; i++){
			         ...
			         html.push('<p class="weui-media-box__desc">身份证'+data[i].idNum+'</p>');
		...
		$(document).on('click','#add',function(){
		   ...
		   location.href=window.BASEPATH + 'pubnum/address/edit';
		});
		
		$(document).on('click','.del',function(){
		var ids=this.id.split('-');
		    var _uriDel = window.BASEPATH + 'pubnum/delAddress?addressId='+ids[1];
		...
		var ids=this.id.split('-');
		    var _uriDelCollect = window.BASEPATH + 'pubnum/setDefAddress?addressId='+ids[1];
		
			$.get(_uriDelCollect, null, function(data){
				data = parseAjaxResult(data);
				if(data === -1) return;
...
<body>
	<div id="page">
		...
		<div class="content">
			...
			<div class="weui-panel__bd" id="addressList" style="padding-bottom:40px;">
	         
	        </div>
	
			<a id="add" style="position:fixed;bottom:2px;width:96%;margin-left:2%;background-color:#18b4ed;height:40px;line-height:40px;" href="javascript:;" class="weui-btn weui-btn_primary">添加地址</a>
