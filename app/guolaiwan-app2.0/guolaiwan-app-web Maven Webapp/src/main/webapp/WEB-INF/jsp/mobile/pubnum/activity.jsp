...
<script type="text/javascript">
	...
	$(function() {
	  ...
	  var parseAjaxResult = function(data){
	   ...
	   var refActivityId='${refActivityId}';
       var activityName;
       var activityPic;
       var activityUrl;
		...
		$(document).on('click','.product',function(){
	       ...
	       var isSurpport=$(this).attr('data');
	       if(!isSurpport||isSurpport==undefined||isSurpport==0){
	          location.href=window.BASEPATH + 'pubnum/product/index/'+codes[1];
	       }else{
	          var actrel=$(this).attr('activityData');
	          location.href=window.BASEPATH + 'pubnum/product/index/surpport/'+actrel+'/0';
	       }
	  ...
	  $(document).on('click','#downloadapp',function(){
		...
		function getProduct(){
			...
			$.get(_uriproduct, null, function(data){
				...
				if(data === -1) return;
				if(data){
				   $('#actImage').attr('src',data.url+data.activity[0].rulepic);
				   $('#actTitle').html(data.bundles[0].title);
				   activityName = data.bundles[0].title + '-过来玩';
				   activityPic = 'http://<%=weburl%>/file' + data.activity[0].rulepic;
				   var html=[];
				   var activitys=data.activity;
				   for(var j=0;j<activitys.length;j++){
				         
				         if(refActivityId&&refActivityId!='0'&&refActivityId==activitys[j].id){
				             <%-- $('#actImage').attr('src','http://<%=weburl%>/file' + activitys[j].rulepic);
				             $('#actTitle').html(activitys[j].name); --%>
							 activityName = activitys[j].name + '-过来玩';
							 activityPic = 'http://<%=weburl%>/file' + activitys[j].rulepic;
				         }
				         ...
				         else if((!refActivityId||refActivityId=='0')&&activitys[j].id==5){
				            continue;
				         }
				         html.push("<tr><td style='padding-left:15px;font-size:13px;font-weight:bold'>"+activitys[j].name+"</td><td></td></tr>")
				         var pros=data[activitys[j].id];
						   for(var i=0;i<pros.length;i++){
						       if(i%2==0){
						          html.push('<tr>');
						       }
			                     html.push('<td style="padding:10px;width:50%" activityData="'+pros[i].activityReId+'" data="'+pros[i].isSurpport+'" class="product" id="pro-'+pros[i].id+'">');
	    ...
	    function doScanShare(){
	        ...
	        wx.ready(function() {
	            ...
	            wx.onMenuShareTimeline({
                            ...
                            link: activityUrl, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
                            imgUrl: activityPic, // 分享图标
                            success: function () {
	...
	<div id="page">
		...
		<div class="header">
			<div class="wrapper">
				...
				<div class="header-content">活动主页</div>
			</div>
		</div>
		<div class="content">
			
			<div class="swiper-container" data-space-between='10' data-pagination='.swiper-pagination' data-autoplay="1000">
			    <div style="height:230px;" class="swiper-slide"><img id="actImage" class="topmod" style="height:200px;" src="" alt="">
				<div style="font-size:12px;position:absolute;padding-left:5px;bottom:0px;color:#FFF"></div>
				
				</div>
			</div>
            <div id="actTitle" style="font-size:12px;margin-left:12px;margin-top:5px;"></div>
