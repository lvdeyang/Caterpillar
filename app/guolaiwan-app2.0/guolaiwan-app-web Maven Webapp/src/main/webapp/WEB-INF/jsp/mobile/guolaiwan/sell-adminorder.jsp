...
<script type="text/javascript">
	...
	$(function() {
	    function getList(){
	        ...
	        $('#order_table_passed').children().remove();
	        $('#order_table_notPassed').children().remove();
	        $('#order_table_payed').children().remove();
	        var _uriQuery = window.BASEPATH + 'distributor/sellorder/list/${distributorId}';
			...
			$.get(_uriQuery, null, function(data){
			    ...
			    if(data && data.length>0){
			       ...
			       for(var i=0;i<data.length;i++){
			           var html=[];
				       html.push('<a href="'+window.BASEPATH + 'distributor/order/checkadmin/'+data[i].id+'"');
					   ...
					   html.push('<h4 style="width:50%;float:left;font-size:12px" class="weui-media-box__title">'+data[i].name+'</h4><h4 style="width:25%;float:right" class="weui-media-box__title"></h4>');
					   html.push('<p style="width:100%;height:15px;font-size:12px" class="weui-media-box__desc">'+data[i].count+'(个)/'+data[i].price+'元</p>');
					   html.push('<p style="width:100%" class="weui-media-box__desc"><div style="float:right;margin-right:38px;font-size:12px;">详情</div></p>');
					   html.push('</div>');
					   html.push('</a>');
				       if(data[i].status=="CHECKED"){
				          $('#order_table').append(html.join(''));
				       }else if(data[i].status=="PASSED"){
				       	  $('#order_table_passed').append(html.join('')); 
				       }else if(data[i].status=="NOTPASSED"){
				          $('#order_table_notPassed').append(html.join(''));
				       }else{
	...
	<div id="page">
		...
		<div class="header">
			<div class="wrapper">
				...
				<div class="header-content">分销订单</div>
			</div>
		</div>
		<div class="content">
			...
			<div class="weui-tab">
				<div class="weui-navbar" style="height:40px;line-height:14px;">
					<a onclick="return false" class="weui-navbar__item weui-bar__item--on" href="#tab1">
						审核中 </a> <a onclick="return false" class="weui-navbar__item" href="#tab2">审核通过</a>
						<a onclick="return false" class="weui-navbar__item" href="#tab3">未通过</a>
						<a onclick="return false" class="weui-navbar__item" href="#tab4">已支付</a>
				</div>
				<div class="weui-tab__bd">
					<div id="tab1" class="weui-tab__bd-item weui-tab__bd-item--active">
                        
                          <div id="order_table" class="weui-panel__bd">
					...
					<div id="tab2" class="weui-tab__bd-item">
                       ...
                       <div id="order_table_passed" class="weui-panel__bd">
                    ...
                    <div id="tab3" class="weui-tab__bd-item">
					 	...
					 	<div id="order_table_notPassed" class="weui-panel__bd">
