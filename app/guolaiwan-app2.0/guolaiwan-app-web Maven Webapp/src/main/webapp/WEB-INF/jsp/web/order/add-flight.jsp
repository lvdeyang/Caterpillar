...
<style type="text/css">
	.order-table{width:50%; margin:auto;}
	.order-table tr{height:40px;}
	.order-table td{text-align:center;}
	.order-table td.left{text-align:left; padding:0;}
	.order-table caption{font-size:16px; font-weight:700;}
</style>
         ...
         return a
            ...
            $("#add").click(function(){
            	...
            	if(pcount==stock){
            		...
            		if(pcount >= parseInt(${driver.passenger})){
            			return;
            		}
            		pcount++;
            		$("#pcount").val(pcount);
...
<body>
	...
	<!-- 订单提交成功 begin-->
	<section>
	  <form action="<%=path%>/user/order/create/flight/order/${flight.uuid}" method="post" id="ordersub" onsubmit="return dosubmit()">
		...
		<div class="containers">
				...
				<div>
					...
					<div class="pc-border">
						<table class="order-table">
							<caption>${flight.flightType=="固定时间"?"班次":flight.flightType}：${route.origin}——${route.end}</caption>
							<tbody>
								<tr>
									<td>发车时间：</td>
									<td class="left">${flight.goTime}</td>
								</tr>
								<tr>
									<td>司机师傅：</td>
								...
								<tr>
									<td>座位数量：</td>
									<td class="left">${driver.passenger}</td>
								</tr>
								<tr>
									<td>单位价格：</td>
									<td class="left">${flight.price}</td>
								</tr>
								<tr>
									<td>出行日期：</td>
									<td class="left pc-label"><input style="width:150px" name="orderBookDate" class="Wdate" type="text" onClick="WdatePicker()" value="${tody}"></td>
								</tr>
								<tr>
									<td>预定数量：</td>
									<td class="left ">
										<span>
											...
											<input type="text" style="width:40px; text-align:center" id="pcount" name="pcount" readonly="readonly" value="1">
								...
								<tr>
									<td>备用电话：</td>
									<td class="left pc-label"><input style="width:265px;" type="text" name="backPhone" placeholder="如果与注册手机号码不同，请填写备用电话" /></td>
								</tr>
								<tr>
									<td>订单留言：</td>
									<td class="left pc-label"><textarea name="orderRemark" style="width:265px; height:80px; padding:6px;"></textarea></td>
								</tr>
							</tbody>
						</table>
						<%-- <div class="pc-order-text clearfix">
			...
			<div class="clearfix">
				<div class="fr pc-list-t">
					<ul>
						<li><span><b id="pcount2">1</b> 件商品，总商品金额：</span> <em>￥<a id="orderAllMoney">${flight.price}</a></em></li>
						<li><span>减额：</span> <em>￥<a id="jmoney">0.00</a></em></li>
						<li><span>运费：</span> <em>￥<a id="yun">0.00</a></em></li>
						<li><span>应付总额：</span> <em>￥<a class="payMoney">${flight.price}</a></em></li>
					</ul>
				</div>
			</div>
			...
			<div class="clearfix">
				<div class="fr pc-space-j">
					<span>应付总额：<strong>￥<i class="payMoney">${flight.price}</i></strong></span>
