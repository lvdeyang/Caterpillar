	...
	$(function() {
      ...
      $("#date").calendar({
         ...
         onDayClick:function(p, dayContainer, year, month, day){
            refreshTable($('#date').val());
         }
      });
      
      function refreshTable(date){
        var _uriRooms = window.BASEPATH + 'pubnum/stock/getRooms?productId=${productId}&date='+date;
		
		$.get(_uriRooms, null, function(data){
			...
			if(data && data.length>0){
			   ...
			   for(var i=0;i<data.length;i++){
			      ...
			      html.push('<div class="room" id="room-'+data[i].id+'" style="background:'+color+'">'+
			      data[i].name+'<br>'+
			      (data[i].userName==null?'':data[i].userName)+'<br>'+
			      (data[i].userTel==null?'':data[i].userTel)+'</div>')
      ...
      function changeStatus(id,status){
	      var _urichange = window.BASEPATH + 'pubnum/stock/changeStatus?roomId='+id+'&status='+status+'&date='+$('#date').val();
			
			$.get(_urichange, null, function(data){
				data = parseAjaxResult(data);
				if(data === -1) return;
				if(data && data.length>0){
				  refreshTable($('#date').val());
      ...
      $(document).on('click','.room',function(){
          ...
          $.modal({
			  ...
			  text: "选择预订或者取消预订",
			  buttons: [
			    { text: "预定", onClick: function(){ 
			        changeStatus(ids[1],1);
			    
			    } },
			    { text: "取消预订", onClick: function(){ 
			    
			        changeStatus(ids[1],0);
	...
	<div id="page">
		...
		<div class="header">
			<div class="wrapper">
				...
				<div class="header-content">预定信息</div>
			</div>
		</div>
		<div class="content">
			  <div class="weui-cell">
		        ...
		        <div class="weui-cell__bd">
		          <input class="weui-input" id="date" value='${date}' type="text" readonly="">
		        </div>
		      </div>
		</div>
		<div id="roomList">
