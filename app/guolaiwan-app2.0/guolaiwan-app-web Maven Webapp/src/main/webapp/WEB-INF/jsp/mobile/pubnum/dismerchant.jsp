...
<script type="text/javascript">
	...
	$(function() {
		...
		function getMerchants(){
	        var _uriMechants = window.BASEPATH + 'pubnum/getDisBymerchant?merchantId=${merchantId}';
	
			
			$.post(_uriMechants, null, function(data){
				if(data){
		...
		function generateProduct(data){
	        ...
	        //后面的
	        html.push('<div style="margin-top:5px;padding-bottom:50px;" class="weui-panel__bd allMer">');
		    for(var j=0;j<merchants.length;j++){
			   ...
			   html.push('<img style="width:100%;height:100%" class="weui-media-box__thumb" src="'+merchants[j].licenseUrl+'">');
			   html.push('</div>');
			   html.push('<div class="weui-media-box__bd">');
			   html.push('<h4 class="weui-media-box__title">电话:'+merchants[j].phone+'</h4>');
			   html.push('<p class="weui-media-box__desc">'+merchants[j].address+'</p>');
	    ...
	    $(document).on("click",".merchant",function(){
	    	...
	    	location.href=window.BASEPATH + 'pubnum/mechant/distributorhome/index?disId='+ids[1];
	    });
	...
	<div id="page">
		...
		<div class="header">
			<div class="wrapper">
				...
				<div class="header-content">分销商列表</div>
			</div>
		</div>
