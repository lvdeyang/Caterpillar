...
public class MerchantDAO extends AbstractBaseDao<MerchantPO> {
	 ...
	 * @return
	 */
	public List<MerchantPO> getMerchantByModularCode(
			String modularCode, 
			int pageNum, 
			int pageSize) {
	 ...
	 * @return count 
	 */
	public int countMerchantByModularCode(
			String modularCode) {
	...
	public List<MerchantDTO> getMerchantByPro(Map<String, Object> nmap,int pageNum ,int pageSize){
		StringBufferWrapper sqlWrapper = new StringBufferWrapper().append("select DISTINCT m.* from t_sys_merchant m , t_sys_modular_product p where m.id = p.productMerchantID");
	...
	public int getMerchantByProC(Map<String, Object> nmap){
		StringBufferWrapper sqlWrapper = new StringBufferWrapper().append("select count(1) c from t_sys_merchant m , t_sys_modular_product p where m.id = p.productMerchantID");
		for (Map.Entry<String,Object> entry : nmap.entrySet()) {
			if(entry.getKey().equals("shopName")){
				sqlWrapper.append(" ");
				sqlWrapper.append("and m.shopName like '%");
	...
	public List<MerchantPO> appfindByCom(Long comId,String modular,String mclass){
		...
		List<MerchantPO> merchants = findByHql(hql);
		if(merchants==null) return new ArrayList<MerchantPO>();
		return merchants;
	}
	...
	public List<MerchantPO> findMerByCom(long comId,int pageNum,int pageSize) {
		...
		if(merchantPOs == null || merchantPOs.size() <= 0) return null;
		return merchantPOs;
	}
}
