...
public class VideoPicDAO extends AbstractBaseDao<VideoPicPO> {
	...
	public List<VPCommentPO> findCommentsPage(VideoPicPO videoPic, int pageNum, int pageSize) {
		...
		List<VPCommentPO> vpComments = getCurrentSession()
				.createFilter(videoPic.getVPComments(), "order by updateTime desc").setFirstResult(firstResult)
	...
	// 封装南山攻略模块
	public List<VideoPicPO> getbymerchantId(Long merchantId) {
		QueryHql hql = newQueryHql();
		hql.andBy("merchantId", Condition.eq, merchantId);
		List<VideoPicPO> videoPicPOs = findByHql(hql);
		return videoPicPOs;
	}
